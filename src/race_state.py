...
def get_session_info(session) -> Dict[str, Any]:
    ...
    session_info = {
        ...
        "GrandPrix": None,
        "Circuit": None,
        "SessionType": getattr(session, "name", None),
    }

    if event is not None:

        try:
            session_info["Season"] = event["EventDate"].year
        except Exception:
            pass

        try:
            session_info["GrandPrix"] = event["EventName"]
        except Exception:
            pass

        try:
            session_info["Circuit"] = event["Location"]
        except Exception:
            pass
...
def get_driver_info(session, driver: str) -> Dict[str, Any]:
    """
    ...
    driver :
        Driver abbreviation, for example:
        VER, HAM, LEC, NOR.
    ...
    try:
        ...
        # FastF1 usually stores driver abbreviations
        # in the Abbreviation column.
        if "Abbreviation" in results.columns:

            driver_rows = results[
                results["Abbreviation"].astype(str).str.upper() == driver
            ]
            ...
            if not driver_rows.empty:
                ...
                if "TeamName" in row.index:
                    result["Team"] = row["TeamName"]

                if "DriverNumber" in row.index:
                    result["DriverNumber"] = row["DriverNumber"]
...
def get_driver_laps(session, driver: str) -> pd.DataFrame:
    ...
    # Convert sector times to seconds if available.
    for column in [
        "Sector1Time",
        "Sector2Time",
        "Sector3Time",
        ...
        if column in laps.columns:
            ...
            laps[f"{column}Seconds"] = (
                laps[column].dt.total_seconds()
            )
...
def get_current_lap(laps: pd.DataFrame) -> Optional[int]:
    """
    ...
    return _safe_int(valid_laps.max())
...
def get_tyre_information(
    ...
    """
    Extract current tyre compound, tyre life,
    ...
    if "Compound" in latest_lap.index:
        ...
        if pd.notna(compound):
            result["TyreCompound"] = str(compound).upper()
    ...
    if "Stint" in laps.columns:
        ...
        valid_stints = laps["Stint"].dropna()

        if not valid_stints.empty:

            current_stint = valid_stints.iloc[-1]

            current_stint_laps = laps[
                laps["Stint"] == current_stint
            ]

            result["CurrentStintLength"] = len(
                current_stint_laps
            )
...
def get_pit_stops(laps: pd.DataFrame) -> int:
    ...
    return max(len(valid_stints) - 1, 0)
...
def get_pace_information(
    ...
    # --------------------------------------------------------
    # Average pace over all valid laps
    # --------------------------------------------------------

    result["AveragePace"] = _safe_float(
        valid_times.mean()
    )
    ...
    if len(valid_times) >= 3:
        ...
        result["AvgPaceLast3"] = _safe_float(
            valid_times.tail(3).mean()
        )

    if len(valid_times) >= 5:

        result["AvgPaceLast5"] = _safe_float(
            valid_times.tail(5).mean()
        )

    if len(valid_times) >= 10:

        result["AvgPaceLast10"] = _safe_float(
            valid_times.tail(10).mean()
        )
...
def estimate_degradation(
    ...
    This is an initial Sprint 1 approximation.

    A more advanced degradation model can be introduced
    in a future sprint.
    ...
    # Use the most recent 10 valid laps.
    recent = valid_laps.tail(10).copy()
    ...
    try:
        ...
        # Linear regression:
        #
        # Lap Time = slope * Lap Index + intercept
        #
        # The slope approximates degradation.
...
def get_race_progress(
    ...
    result["LapsRemaining"] = max(
        total_laps - current_lap,
        0,
    )

    result["RaceProgress"] = _safe_float(
...
def build_race_state(
    ...
    if laps.empty:
        ...
        raise ValueError(
            f"No valid lap data found for driver: {driver}"
        )
